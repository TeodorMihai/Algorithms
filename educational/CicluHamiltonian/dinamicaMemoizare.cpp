	...
	if(D[conf][last] == -1) {
		...
		for(unsigned i = 0 ; i < G[last].size(); ++i)
			if((1 << G[last][i]) & conf) {
				...
				//cum eu merg doar de la configuratii cu mai multe noduri spre configuratii cu mai putine, pot fi sigur ca conf contine nodul
				//las astfel expresia conf ^ (1 << last) are ca scop scoaterea lui din configuratie
				//calc(i, conf, j) = costul minim al lantului de la nodul i la nodul j cu nodurile din conf inclusiv
			}
	}
...
void solve() {
	...
	//e un ciclu pot porni din orice nod vreau
