...
const double MAXC = 1e5 + 100;//pentru punct in poligon, optional
const int NMAX = 102;
...
struct Poligon {
	...
	int dett(const Point& p, const Point& p1, const Point& p2) {
		double x = p.x;
		double y = p.y;
		return x * p1.y + p1.x * p2.y + p2.x * y -
			   y * p1.x - p1.y * p2.x - p2.y * x > eps ? 1 : -1;
			   //nu merge pt eps = 0 ?
	}

	//se taie poligonul cu dreapta seg, se pastreaza in functie de sgn
	Poligon cut(Segment seg, int sgn) {

		Poligon pol;

		for(unsigned i = 0 ; i < v.size() - 1; i++) {

			if( dett(seg.p1, seg.p2, v[i]) == sgn) {//primul pct interior
				
				pol.v.push_back(v[i]);
				if(dett(seg.p1, seg.p2, v[i + 1]) == -sgn)  {//al doilea pctexterior
				
					Point inter = Line(seg.p1, seg.p2).intersection(Line(v[i], v[i + 1]));
					pol.v.push_back(inter);//bag intersectia lor
				}
			} else if(dett(seg.p1, seg.p2, v[i + 1]) == sgn) {

					Point inter = Line(seg.p1, seg.p2).intersection(Line(v[i], v[i + 1]));
					pol.v.push_back(inter);
			}
		}
	...
	/*poligonul format prin taierea lui cu dreapta line */
	Poligon computeInter(Poligon poli1, int sgn) {

		v.push_back(v[0]);

		for(unsigned i = 0 ; i < v.size() - 1; ++i) {
			Segment seg(v[i], v[i + 1]);

			if(poli1.v.size() <= 2)
				return poli1;

			poli1.v.push_back(poli1.v[0]);
			poli1 = poli1.cut(seg, sgn);
		}
...
int main() {
	...
	Poligon pol;
	//sens trigonometric neaprat
	pol.v.push_back(Point(MAXC, -MAXC));
	pol.v.push_back(Point(MAXC, MAXC));
	pol.v.push_back(Point(-MAXC, MAXC));
	pol.v.push_back(Point(-MAXC, -MAXC));
	...
	fout << max( poli2.computeInter(pol, -1).computeArea() ,
				 poli2.computeInter(pol, 1).computeArea()) << '\n';
